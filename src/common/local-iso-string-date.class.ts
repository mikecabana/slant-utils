export class LocalIsoStringDate {
  ...
  public static convert(date: Date, options: { includeOffset: boolean } = { includeOffset: false }): string {
    let localIsoDate = `${LocalIsoStringDate.pad(date.getFullYear())}-${LocalIsoStringDate.pad(
      date.getMonth() + 1,
    )}-${LocalIsoStringDate.pad(date.getDate())}T${LocalIsoStringDate.pad(date.getHours())}:${LocalIsoStringDate.pad(
      date.getMinutes(),
    )}:${LocalIsoStringDate.pad(date.getSeconds())}.${LocalIsoStringDate.padMillis(date.getMilliseconds())}`;
